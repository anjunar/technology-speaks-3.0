...
class PostController(val identityHolder: IdentityHolder, val entityManager : EntityManager) extends EntityManagerProvider {
  ...
  def update(@RequestBody post: Post): Data[Post] = {
    val managedPost = post.merge()
    val data = new Data(managedPost, SchemaHateoas.enhance(managedPost, Post.schema))
    ...
    managedPost.addLinks(
      LinkBuilder.create[PostCommentsController](_.comments(new PostCommentSearch(managedPost)))
        .build(),
      LinkBuilder.create[PostCommentController](_.save(managedPost, null))
        .build()
    )

    if (managedPost.user == identityHolder.user) {
      managedPost.addLinks(
        LinkBuilder.create[PostController](_.read(managedPost))
          .build(),
        LinkBuilder.create[PostController](_.update(null))
          .build(),
        LinkBuilder.create[PostController](_.delete(managedPost))
          .build()
      )
    }
