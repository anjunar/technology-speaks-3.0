...
class PostCommentController(val identityHolder: IdentityHolder, val entityManager : EntityManager) extends EntityManagerProvider {
  ...
  def save(@PathVariable("id") post: Post, @RequestBody body: FirstComment): Data[FirstComment] = {
  ...
  @PutMapping(value = Array("/timeline/posts/post/{id}/comment"), produces = Array("application/json"), consumes = Array("application/json"))
  @RolesAllowed(Array("User", "Administrator"))
  def update(@PathVariable("id") post: Post, @RequestBody body: FirstComment): Data[FirstComment] = {
    for (comment <- body.comments.asScala if isUserEmpty(comment)) {
      comment.user = identityHolder.user
    }
  ...
  @RolesAllowed(Array("User", "Administrator"))
  def delete(@PathVariable("id") post: Post, @RequestBody body: FirstComment): ResponseEntity[Void] = {
